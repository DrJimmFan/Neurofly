...
- Default seed-42 pick/place completed in about 7.5 simulation seconds. Actual final object state is in `examples/pick-and-place.nvfly.json`.
- Barrier preset stops, excludes the blocked target, selects the ball and completes placement. Its real state recording is in `examples/obstruction.nvfly.json`.
...
- Browser imported the supplied barrier recording. Export created `Barrier_recovery.nvfly.json` in Downloads; its parsed content matched the imported example exactly at that verification point.
